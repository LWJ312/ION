...
class IRGSemMap(torch.nn.Module):
    def __init__(self, args):
        action_space = args.action_space
        self.num_cate = args.num_category
        resnet_embedding_sz = args.hidden_state_sz
        hidden_state_sz = args.hidden_state_sz
        super(IRGSemMap, self).__init__()
        self.sem_info = nn.Linear(49,64)
        self.update_map = nn.Linear(64+256+10,256)
        self.map_embedding = nn.Linear(256,49)
        self.map = None
        ...
        pointwise_in_channels = 64 + self.num_cate + 10+32 + 29
    ...
    def embedding(self, state, target, action_embedding_input):
        ...
        masks = target['masks'] 
        action_info = action_embedding.repeat(masks.shape[0],1)
        masks = masks.view(masks.shape[0],masks.shape[1]*masks.shape[2])
        sem_info = F.relu(self.sem_info(masks))
        map_info = torch.cat((self.map.detach(),sem_info,action_info.detach()),dim=1)
        sem_map = F.relu(self.update_map(map_info))
        self.map = sem_map
        sem_map_embedding = F.relu(self.map_embedding(sem_map)).view(1,29,7,7)
        ...
        x = torch.cat((x, target_embedding, action_reshaped,glove_reshaped,sem_map_embedding), dim=1)
    ...
    def forward(self, model_input, model_options):
        ...
        if not (action_probs!=0).sum():
            self.map = torch.zeros(29,256).to(action_probs.device)
