...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    def reset(self, scene_name=None):
        ...
        if scene_name != self.scene_name:
            ...
            with open(
                    os.path.join(
                        self.offline_data_dir, self.scene_name, 'cate_visible_location_bbox.json'
                    ),
                    'r',
            ) as f:
                self.cls_metadata = json.load(f)
            ...
            self.cls_masks= self.h5py.File(os.path.join(
                    self.offline_data_dir, self.scene_name, 'class_masks.hdf5'
                ),'r')
    ...
    def shortest_path_to_target(self, source_state, objId, get_plan=False):
        ...
        states_where_visible = [
            self.get_state_from_str(*[float(x) for x in str_.split('|')[:4]])
    ...
    def compute_iou(self,box1,box2):
        in_x = min(box1[2],box2[2]) - max(box1[0],box2[0])
        in_y = min(box1[3],box2[3]) - max(box1[1],box2[1])
        inter = 0 if in_x < 0 or in_y < 0 else in_x * in_y
    ...
    def object_is_visible(self, objId):
        if self.using_raw_metadata:
            ...
            if self.metadata_file_name.split('_')[0] == 'ins':
                result = False
                gt = []
                for loc_bbox in self.metadata[objId]:
                    if loc_bbox[:len(str(self.state))] == str(self.state):
                        gt.append(loc_bbox.split('|')[-1])
                # print(self.select)
                for bbox in gt:
                    result = True
                    if self.select:
                        result = False
                        bbox = [int(i)/300.0 for i in bbox.split('-')]
                        iou = self.compute_iou(bbox,self.select_bbox)
                        if iou > 0.9:
                            result=True
            else:
                result = str(self.state) in self.metadata[objId]
            return result

    def cls_success(self, objId):
        result = False
        for loc_bbox in self.cls_metadata[objId]:
            if loc_bbox[:len(str(self.state))] == str(self.state):
                result = True
        return result
    ...
    def get_detection_feature(self):
        try:
            df = self.detection_feature[str(self.state)]
        except KeyError:
            df = None
        return df
