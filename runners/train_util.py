...
import numpy as np
from utils.model_util import gpuify, toFloatTensor,toLongTensor
...
def get_other_metrics(player):
    metrics = {
        'ep_length (TNA)': player.eps_len,
        'move_dis (APL)': player.episode.move_count * 0.25,
        'ep_area (AA)': len(player.episode.ep_area) / len(player.episode.environment.controller.grid),
        'dis2target (MD)': player.episode.dis2tar,
    }
    if player.success:
        metrics['foj (CF)'] = player.episode.success_foj
